...
export const Sidebar: React.FC = () => {
  ...
  const { currentUser, users, loginAsLeaderboardName, activeGtVersion } = useAppStore();
  ...
  const userRole = currentUser?.role || (currentUser?.leaderboardName === "Rijal" ? "Ketua Tim" : "Anggota Tim");

  return (
    <aside
      style={{
        background: "linear-gradient(180deg, #0e1732 0%, #111a3d 50%, #152252 100%)",
    ...
    >
        ...
        {!isCollapsed && (
          <Link href="/dashboard" className="flex items-center gap-3.5 min-w-0">
            ...
            <div className="flex flex-col leading-tight min-w-0">
              <span style={{ fontSize: "14.5px", fontWeight: 900, color: "#fff", letterSpacing: "-0.4px" }}>
                VisionAI Studio
              </span>
              <span style={{ fontSize: "10px", fontWeight: 700, color: "rgba(255,255,255,0.45)", letterSpacing: "0.08em", textTransform: "uppercase" }}>
                Evaluasi Model CV
              </span>
            </div>
          </Link>
      ...
      {/* Section Header */}
      {!isCollapsed && (
        <div style={{ padding: "22px 24px 8px", fontSize: "10.5px", fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.35)" }}>
          Navigation
        </div>
      )}

      {/* Navigation Items (White Floating Capsule for Active Item) */}
      <nav className="flex-1 overflow-y-auto" style={{ padding: isCollapsed ? "12px 10px" : "8px 16px 16px" }}>
        ...
        {!isCollapsed ? (
          <div className="relative">
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <div style={{ textAlign: "left" }}>
                  <div style={{ fontSize: "12.5px", fontWeight: 800, color: "#fff" }}>{userName}</div>
                  <div style={{ fontSize: "10.5px", color: "rgba(255,255,255,0.45)" }}>{userRole}</div>
                </div>
              </div>
              {isAccountMenuOpen ? (
                <ChevronUp style={{ width: "16px", height: "16px", color: "rgba(255,255,255,0.5)" }} />
              ) : (
            ...
            {isAccountMenuOpen && (
              ...
              >
                <div style={{ fontSize: "10px", fontWeight: 700, color: "rgba(255,255,255,0.4)", padding: "4px 8px" }}>
                  PILIH AKUN PENGGUNA
                </div>
                <div className="space-y-1 mt-1">
                  {users.map((u) => {
                    ...
                    return (
                      ...
                      >
                        <span style={{ fontWeight: active ? 700 : 500 }}>{u.leaderboardName}</span>
                        {active && <Check style={{ width: "14px", height: "14px", color: "#60a5fa" }} />}
                      </button>
                    );
                  })}
                </div>
